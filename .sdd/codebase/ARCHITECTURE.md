...
- **`Emend/Shell/MainWindow.swift`** — Three-pane layout: editor (left), file tree (center), info sidebar (right). Skeletons during Phase 2; filled in by later user stories.
...
- `Screens/` — Pane views (editor, sidebar, etc.)
...
User types in NSTextView
    ↓
SwiftUI onChange hook captures {utf16Range, replacement}
    ↓
Calls Rust `push_edit(doc_handle, range, text)` — **synchronous, off-main-thread**
    ↓
...
Swift updates NSTextStorage (resets insertion point, etc.)
    ↓
NSTextView triggers highlight via delegate
    ↓
(Phase 2+) Calls Rust `highlight_range(doc_handle, range)` — **off-main-thread**
    ↓
Returns tree-sitter highlight tokens
    ↓
Swift updates NSTextView attributes
```
...
| (coming) `open_document(path, size_limit) -> Result<DocHandle, FfiError>` | Create a document handle | Editor initialization |
| (coming) `push_edit(handle, utf16Range, replacement) -> Result<(), FfiError>` | Apply keystroke | Per-keystroke sync path |
| (coming) `highlight_range(handle, range) -> Result<Vec<Token>, FfiError>` | Incremental highlight | Syntax coloring |
...
|------------|----------|---------|-------|
| **Document buffer** | Rust `Document` (ropey shadow) | Owned by Rust; Swift updates send tiny deltas | Hot path; kept in sync via `push_edit` |
| **Text attributes** | Swift `NSTextStorage` | Source of truth for display; Rust mirrors in shadow rope | Edits flow: user → NSTextStorage → Rust |
...
| **Open-document list** | Swift app state | Registry of handles | Tracks which Rust `Document` handles are live |
...
| **Incremental syntax highlight** | tree-sitter (editor, advisory) vs. comrak (preview, authoritative) | `crates/emend-core` (Phase 1+) |
